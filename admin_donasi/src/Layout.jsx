...
export default function Layout () {
	...
	<div className="container">
		...
		<Link class="navbar-brand" to="/">
			<img src="https://cdn.pixabay.com/photo/2014/04/02/16/18/hands-306885_1280.png" width="45" height="30" class="d-inline-block align-top" alt=""/>
		</Link>
		<div class="collapse navbar-collapse">
    		<ul class="navbar-nav mr-auto">
				...
				<li class="nav-item">
					...
					<div className={color ? "text-white" : "text-black"}>Donasi </div> 
					</Link>
				</li>
				<li class="nav-item">
					<Link to="/Profile" class="nav-link" > 
					<div className={color ? "text-white" : "text-black"}>Profile </div> 
					</Link>
				</li>
	  		</ul>
	  	</div>
			<p className="text-white">{profile.email}</p>
			<button onClick={handleShow} class="btn btn-primary mx-2">Login
			</button>
			<button onClick={changeColor} className={color ? "btn btn-warning" : "btn btn-secondary"}>
				{color ? 'Yellow' : 'Dark'}
			</button>
			
			<img src={profile.image} width={50} onClick={logout}/>
		</nav>
	</div>
	</div>
	<Outlet />
